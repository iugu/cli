...
call <app id|tag> <action> [--arg k=v …] [--input '{…}'] [--workspace <id>]   (an action of a provider app, as you, through Console's bridge — Billing: `iugu call billing get_plan|set_prices|preview_pricing|publish_version|get_revenue|list_invoices …`; elevated actions are confirmed by the person on a Console page)
